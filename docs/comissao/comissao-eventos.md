...
```mermaid
flowchart TD
    A[Compor comissao de eventos] --> B[Alinhar calendario com Ensino]
    B --> C[Mapear eventos existentes]
    C --> D[Criar calendario anual]
    D --> E[Convidar grupos de pesquisa]
    E --> F[Definir eventos prioritarios]
    F --> G[Planejar comunicacao e infraestrutura]
    G --> H[Executar eventos]
